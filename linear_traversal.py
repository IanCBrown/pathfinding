        ...
        for column in range(Grid.width):
            for row in range(Grid.height):
                ...
                pygame.draw.rect(screen, color, [(Grid.rect_margin + Grid.rect_width) * column + Grid.rect_margin,
                                               (Grid.rect_margin + Grid.rect_width) * row + Grid.rect_margin, Grid.rect_width, Grid.rect_height])

        start_x = Grid.startX
        start_y = Grid.startY

        while start_x < Grid.endX and start_y < Grid.endY:
            pygame.draw.rect(screen, Grid.GREEN, [(Grid.rect_margin + Grid.rect_width) * start_x + Grid.rect_margin,
                                                (Grid.rect_margin + Grid.rect_width) * start_y + Grid.rect_margin, Grid.rect_width, Grid.rect_height])
            start_x += 1
            start_y += 1
